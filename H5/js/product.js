			...
			$.ajax({
				...
				success: function(res) { // res就是后台接口返回的数据
					...
					for (var i = 0; i < fea.length; i++) {
						var feaI = fea[i];
						console.log(feaI);
						featur += ' <div class="Bright-spot-1">' +
							' <div class="Bright-spot-1-l">' +
							'<img src=' + feaI.featureImg + '/>' +
							'</div>' +
							' <div class="Bright-spot-1-r">' +
							'<span>' + feaI.featureDesc + '</span>' +
							'</div>' +
							'</div>';
					}
					...
					for (var i = 0; i < recom.length; i++) {
						var recomI = recom[i];
						console.log(recomI);
						recomend += '<div class="like-1">'+
							'<div class="like-1-img">'
							+'<img src=' + recomI.recommendCover + '/>'+
							'</div>'
							+'<p>' + recomI.recommendDesc +'</p>' +
							'<p style="color: red;">' + recomI.recommendPrice + '元/份' +'</p>'+
							'</div>';
					}
